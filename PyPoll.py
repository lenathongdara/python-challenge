...
with open (output_file, "w") as txt_file:
    ...
    for candidate in candidate_votes:
        ...
        #  Calculate the amount of votes and the percentage 
        votes = candidate_votes.get(candidate)
        percentage = float(votes) / float(total_votes) * 100 

        # Find the amount of votes the winning candidate gets 
        if (votes > winning_votes):
            winning_votes = votes 
            winner = candidate
        
        # Find the total amount of votes and then print it out 
        voter_result = f"{candidate}: {percentage:.3f}% ({votes})\n"
        print(voter_result)

        # Convert into a text file 
